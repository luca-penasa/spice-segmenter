...
class Evaluator:
    ...
    3. :exc:`NotImplementedError` with a descriptive message.
    """
    ...
    def evaluate(self, prop: Property, time: TIMES_TYPES) -> Any:
        """Evaluate *prop* at one or more times.

        Dispatches to :meth:`evaluate_scalar` for a single time value and to
        :meth:`evaluate_vector` for array-like inputs.  Array dispatch is
        controlled by :attr:`~spice_segmenter.support.config.Config.use_vectorized_calls`.
        """
        ...
        # Non-vectorised path: call evaluate_scalar element-wise so the fallback
        # chain is honoured for each call.
    ...
    def evaluate_scalar(self, prop: Property, time_et: float) -> Any:
        ...
        Fallback chain:
        1. Engine scalar fn registered for ``type(prop)``.
        2. ``prop._call_scalar(time_et)`` (transitional).
        3. :exc:`NotImplementedError`.
        """
        try:
        ...
        # Transitional fallback: property still implements _call_scalar
        try:
        ...
        raise NotImplementedError(
            ...
            f"  engine.register({type(prop).__name__}, scalar_fn=your_fn)"
        )
    ...
    def evaluate_vector(self, prop: Property, times_et: np.ndarray) -> np.ndarray:
        ...
        Fallback chain:
        1. Engine vector fn (or auto-vectorised scalar fn via engine).
        2. ``prop._call_vector(times_et)`` (transitional).
        3. :exc:`NotImplementedError`.
        """
        try:
    ...
    def as_spice_boolean_function(
        ...
        The returned object can be passed directly to ``spiceypy.gfudb``.

        Args:
            prop: A boolean property.
            invert: When ``True``, the returned callback negates the result.
        """
        if invert:
            def _fn(udfun: Any, t: float) -> bool:
                return not self.evaluate_scalar(prop, float(t))
        else:
            def _fn(udfun: Any, t: float) -> bool:
                return bool(self.evaluate_scalar(prop, float(t)))
