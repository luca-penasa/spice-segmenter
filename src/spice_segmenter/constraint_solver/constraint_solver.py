...
from spice_segmenter.constant import BoolConstant, Constant
from spice_segmenter.constraint import (
    ConstraintBase,
    ConstraintTypes,
)
from spice_segmenter.occultation import OccultationTypes
from spice_segmenter.ops import Inverted
from spice_segmenter.property_base import BooleanProperty, PropertyTypes
from spice_segmenter.search_reporter import (
    NoSearchReporter,
    SearchReporter,
...
class SpiceEventSolverConfigurator:
    ...
    def set_from_dict(self, pars: dict) -> None:
        ...
        if not is_min_max:
            refval = pars.get("reference_value")
            if refval is None:
                raise ValueError(
                    "No reference value found, and not a minmax constraint",
                )

            refval_unit = pars.get("reference_value_unit")
            property_unit = pars.get("property_unit")
