...
def test_unit_adaptor() -> None:
    ...
    dkm = d(t1)
    dm = pint.Quantity(dkm, "km").to("m").magnitude
    ...
    ph_rad = ph(t1)
    ph_deg = pint.Quantity(ph_rad, "rad").to("deg").magnitude
    assert ph_deg == approx(np.rad2deg(ph_rad))
...
def test_angular_size_constraint() -> None:
    ...
    result_rad = ang_size(t1)
    assert result_rad > 0

    result_deg = pint.Quantity(result_rad, ang_size.unit).to("deg").magnitude
    assert result_deg > 0
...
def test_unit_adaptor_chaining() -> None:
    """Test unit conversion round-trip using pint directly."""
    d = Distance(tc.spacecraft, tc.target)

    result_km = d(t1)
    result_m = pint.Quantity(result_km, "km").to("m").magnitude
    result_km_again = pint.Quantity(result_m, "m").to("km").magnitude

    assert result_km == approx(result_km_again)
